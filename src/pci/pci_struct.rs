            ...
            if let Some(mut node) = self.get_node() {
                ...
                self.next(match node.config_value.get_class().0 {
                    ...
                    0x6 if node.config_value.get_class().1 == 0x4 => {
                        ...
                        // secondary bus number instead of calculating our own. The firmware
                        // (UEFI/BIOS) has already assigned correct bus numbers, and our
                        // calculated bus may not match (e.g., firmware may skip bus numbers
                        // for subordinate bus reservation).
                        ...
                        let next_bus = {
                            ...
                            if fw_secondary != 0 {
                                ...
                                // Firmware didn't program this bridge, fall back to calculated
                                parent.subordinate_bus + 1
                            }
                        };
